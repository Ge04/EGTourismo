const formularioTransporte = document.querySelector("#transportForm");
formularioTransporte.addEventListener("submit", function (e) {
    e.preventDefault(); // Prevent the default form submission
    // console.log("Form submitted");
    const formData = new FormData(formularioTransporte);
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "./insertarTrans.php", true);

    xhr.onload = function () {
        if (xhr.readyState = 4 && xhr.status == 200) {
            formularioTransporte.reset(); 
            const response = JSON.parse(xhr.responseText);
        //     console.log(xhr.response);
            if (response) {
                Swal.fire({
                    icon: 'success',
                    title: 'Transporte registrado correctamente',
                    showConfirmButton: false,
                    timer: 1500
                });
                formularioTransporte.reset(); // Reset the form
                CargarDatos(); // Reload the data
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Error al registrar el transporte',
    ...
    xhr.onload = function () {
        ...
        for (let data of datos) {
            tabla.innerHTML += `
             ...
             <td>${data.precio}</td> 
             <td>
                <button class="btn btn-danger" onclick="EliminarTransporte(${data.id})"><i class="fas fa-trash"></i></button></td> 
             <td>
...
setInterval(CargarDatos,3000); // Refresh data every 3 seconds
CargarDatos();
